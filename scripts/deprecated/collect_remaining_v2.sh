...
collect_post() {
    ...
    # 使用 feed_id (不是 note_id) 调用API
    local detail_output
    if ! detail_output=$(cd "$XHS_DIR" && ./mcp-call.sh get_feed_detail "{\"feed_id\": \"$note_id\", \"xsec_token\": \"$xsec_token\", \"load_all_comments\": true}" 2>&1); then
        echo "  ❌ 获取详情失败: $note_id"
        return 1
    fi
