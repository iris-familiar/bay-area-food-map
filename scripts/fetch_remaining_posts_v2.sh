...
while IFS='|' read -r note_id xsec_token; do
    ...
    # Fetch post detail using the correct script path with xsec_token
    ./scripts/post-detail.sh "$note_id" "$xsec_token" > "$DATA_DIR/posts/2026-02-16/${note_id}.json" 2>&1
    
    # Fetch comments using the correct script path with xsec_token
    ./scripts/comment.sh "$note_id" "$xsec_token" > "$DATA_DIR/comments/2026-02-16/${note_id}.json" 2>&1
